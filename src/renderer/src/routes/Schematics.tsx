...
export function Schematics(): JSX.Element {
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h2 className="text-xl font-semibold text-slate-100">Schematics</h2>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <span className="text-xs text-slate-400 tabular-nums ml-auto">
          {loading ? "…" : (
            <>
              <span className="text-slate-300">{filtered.length}</span> of {allSchematics.length}
            </>
          )}
        </span>
      </div>
              ...
              filtered.map((s) => (
                ...
                >
                  <td className="px-3 py-2">
                    ...
                    <div className="text-xs text-slate-600 font-mono">{s.id}</div>
                  </td>
                  ...
                  <td className="px-3 py-2 text-slate-400 text-xs font-mono">
                    {s.craftingTab ?? <span className="text-slate-700">—</span>}
                  </td>
                  <td className="px-3 py-2 text-right text-slate-300 tabular-nums">{s.complexity}</td>
